...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterMovement : CharacterAbility
    {
        ...
        //Michi Eweiterung
        public GameObject[] FootprintPrefabs; // Array von Fußspur-Prefabs
        public float FootprintOffsetRange = 0.5f; // Maximaler Offset für die Fußspurposition
        private int _stepCount = 0; // Zähler für die Anzahl der Schritte
        public int StepsPerFootprint = 20;  // Anzahl der Schritte pro Fußspur
        ...
        protected void CreateFootprint()
        {
            ...
            {
                // Zufällige Auswahl eines Fußspur-Prefabs
                GameObject selectedFootprintPrefab = FootprintPrefabs[Random.Range(0, FootprintPrefabs.Length)];

                // Erstellen einer neuen Fußspur an einer leicht verschobenen Position
                Vector3 footprintPosition = transform.position + new Vector3(Random.Range(-FootprintOffsetRange, FootprintOffsetRange), 0, Random.Range(-FootprintOffsetRange, FootprintOffsetRange));
                Instantiate(selectedFootprintPrefab, footprintPosition, Quaternion.identity);

                FootprintData data = new FootprintData();
                data.position = footprintPosition;
                data.rotation = Quaternion.identity; // Oder die tatsächliche Rotation, falls erforderlich
                data.prefabIndex = Random.Range(0, FootprintPrefabs.Length);
                allFootprints.footprints.Add(data);




                // Hinweis: Hier wird die Fußspur nicht automatisch zerstört
            }
        }
        //Michi Eweiterung
        ...
        protected virtual void HandleMovement()
        {
            ...
            if (_controller.CurrentMovement.magnitude > IdleThreshold)
            {
                ...
                {
                    CreateFootprint();
                    _stepCount = 0; // Zurücksetzen des Schrittzählers
                }
            }
            else
            {
                _stepCount = 0; // Zurücksetzen des Schrittzählers, wenn der Charakter steht
            }
        ...
        public void LoadFootprints()
        {
            if (File.Exists(GetFilePath()))
            {
                ...
                FootprintDataList data = JsonUtility.FromJson<FootprintDataList>(jsonData);

                foreach (var footprint in data.footprints)
                {
        ...
        {
            return Path.Combine(Application.persistentDataPath, "footprints.json");
        }
